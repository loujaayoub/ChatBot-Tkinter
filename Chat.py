...
import threading
import struct
from customtkinter import CTkColorManager
...
class ChatScreen(tk.Canvas):
    def __init__(self, parent, frame):
        ...
        send_button = customtkinter.CTkButton(master=self,
                                text="Envoyer",
                                ...
                                text_font="lucida 12 bold",
                                corner_radius=6)
        #send_button = tk.Button(self, text="Envoyer", padx=10,
         #                       relief="solid", bd=2, command=self.sent_message_format)

        send_button.place(x=370, y=630)
    ...
    def received_message_format(self, message):
        ...
        t_label = tk.Label(m_frame, bg="#46abbd",fg="white", text=datetime.now().strftime('%H:%M'), font="lucida 7 bold",
